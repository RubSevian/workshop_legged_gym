...
class Go2(LeggedRobot):
    ...
    def _reward_hip_pos(self):
        ...
        return torch.exp(-0.5*error / self.cfg.rewards.tracking_sigma)  # [num_envs]
    ...
    def _reward_com_over_support(self):
        ...
        # base_pos1 = self.root_states[:, 0:3]
        # print(f"base_pos1 {base_pos1}")
        ...
        # return torch.exp(-8.0 * error)
            # ошибка проекции
        error_xy = torch.norm(base_pos[:, 0:2] - support_center[:, 0:2], dim=1)
        #print(f"error_1  {error }")
        #print(f"error_xy  {error_xy }")
        return torch.exp(-error_xy / 0.1)  # чем ближе COM к линии опоры, тем выше награда
    

    def _reward_rear_feet_contact_and_air(self):
    # Детектируем контакты только по задним лапам
        ...
        undesired_penalty = -10 * undesired
    ...
    def _reward_smoothness(self):
        ...
        return 0.1*term_1 + 0.05*term_2 + term_3
