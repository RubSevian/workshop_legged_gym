...
class Go2_Walk(LeggedRobot):
    ...
    def _reward_tracking_pitch(self):
        # Tracking pitch to maintain vertical torso
        base_quat = self.root_states[:, 3:7]  # [num_envs, 4]
        euler = get_euler_xyz(base_quat)  # Returns tuple (roll, pitch, yaw), each of shape [num_envs]
        roll = euler[0]  # [num_envs]
        pitch = euler[1]  # [num_envs]
        episode_time_buf = self.episode_length_buf * self.dt  # [num_envs]
        pitch_command = episode_time_buf * self.cfg.commands.pitch / self.cfg.commands.standup_duration  # [num_envs]
        pitch_command = torch.clip(pitch_command, self.cfg.commands.pitch, 0.)  # [num_envs]
        error = torch.square(pitch_command - pitch) + torch.square(self.cfg.commands.roll - roll)  # [num_envs]
    ...
    def _reward_hip_pos(self):
        # Penalize deviation of all hip joints from default positions
    ...
    def _reward_feet_contact(self):
        # Reward contact for RR_foot and RL_foot, heavily penalize contact for FL_foot, FR_foot, thighs, and calves
        contact = self.contact_forces[:, :, 2] > 1.0  # Check z-axis contact force
        desired_contact = torch.sum(contact[:, self.desired_contact_indices], dim=1)
        undesired_contact = torch.sum(contact[:, self.undesired_contact_indices], dim=1)
        return desired_contact - 5.0 * undesired_contact  # Reward desired contacts, heavily penalize undesired
