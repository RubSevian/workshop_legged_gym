...
from .tumbler_base import LeggedRobot
from legged_gym import LEGGED_GYM_ROOT_DIR
from .go2_tumbler_config import Go2TumblerCfg
...
class Go2Tumbler(LeggedRobot):
    ...
    def __init__(self, cfg, sim_params, physics_engine, sim_device, headless):
        ...
        expected_feet = [f"{leg}_foot" for leg in ("FL", "FR", "RL", "RR")]
        if list(self.dof_names) != expected_dofs:
            raise RuntimeError(
                f"Unexpected Go2 joint order: {self.dof_names}; expected {expected_dofs}"
            )
        if list(self.foot_name) != expected_feet:
            raise RuntimeError(
                f"Unexpected Go2 foot order: {self.foot_name}; expected {expected_feet}"
            )
        print("[go2_tumbler] joint order:")
        for index, name in enumerate(self.dof_names):
            print(f"  {index} -> {name}")
        print(f"[go2_tumbler] feet: {self.foot_name}")
