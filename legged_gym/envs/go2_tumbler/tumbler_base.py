...
from .tumbler_base_task import BaseTask
from .tumbler_terrain import Terrain
from legged_gym.utils.math import quat_apply_yaw, wrap_to_pi, torch_rand_sqrt_float
from legged_gym.utils.helpers import class_to_dict
from .tumbler_base_config import LeggedRobotCfg
...
class Agent:
    def __init__(self, urdf_path):
        if URDF is None:
            raise RuntimeError("urdfpy is required only for morph_dog assets")
...
class LeggedRobot(BaseTask):
    def __init__(self, cfg: LeggedRobotCfg, sim_params, physics_engine, sim_device, headless):
        ...
        self.ref_log = False
        reference_rewards = class_to_dict(self.cfg.rewards.scales)
        self.reference_motion_enabled = self.ref_log or any(
            name.startswith("tracking_ref") and scale != 0
            for name, scale in reference_rewards.items()
        )
        if not self.reference_motion_enabled:
            return
    ...
    def _get_env_origins(self):
        ...
        """
        # ``stair`` is generated as a triangle mesh in ``create_sim`` as well.
        # It must therefore use the per-tile terrain origins.  Treating it as a
        # flat-grid environment places most parallel environments outside the
        # generated mesh, where they fall indefinitely.
    ...
    def _parse_cfg(self, cfg):
        ...
        if self.cfg.terrain.mesh_type not in ['heightfield', 'trimesh', 'stair']:
