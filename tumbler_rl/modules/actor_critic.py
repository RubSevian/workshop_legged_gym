...
class ActorCritic(nn.Module):
    ...
    def _actor_critic(self, obs_dict):
        ...
        expected_history_width = self.dm_encoder.encoder[0].in_features
        if obs_his.shape[-1] != expected_history_width:
            raise ValueError(
                "Invalid proprio_hist width: policy expects "
                f"{expected_history_width} values ({self.HistoryLen} x "
                f"{self.num_obs}), but environment provided {obs_his.shape[-1]}."
            )
