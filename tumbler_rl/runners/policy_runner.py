...
from tumbler_rl.utils.utils import export_policy_as_jit
import torch

from tumbler_rl.algorithms import PPO
from tumbler_rl.modules import ActorCritic, ActorCriticRecurrent
from tumbler_rl.env import VecEnv

from tumbler_rl.modules.actor_critic import DmEncoder


class ImiPolicyRunner:
    ...
    def __init__(self,
        ...
        env_history_len = getattr(self.env, "num_histroy_obs", None)
        if env_history_len is not None and env_history_len != self.HistoryLen:
            raise ValueError(
                "History length mismatch: environment produces "
                f"{env_history_len} observations, but Encoder.HistoryLen is "
                f"{self.HistoryLen}. Set both values to the checkpoint history length."
            )
    ...
    def load(self, path, load_optimizer=True):
        # Checkpoints are commonly trained on CUDA and later inspected on a
        # CPU-only machine.  Loading onto the runner device keeps playback
        # portable instead of failing before the policy is constructed.
        loaded_dict = torch.load(path, map_location=self.device)
        path_1 = "/home/exiao/adaptive_legged_gym/outputs/random_dog/Imi/none_smotthbess/stage1_nn/last.pt"
        path_2 = "/home/exiao/adaptive_legged_gym/outputs/random_dog/Imi/none_Stabilization/stage1_nn/last.pt"
        average_policy = False
        # for key, _ in loaded_dict['dm_encoder_state_dict'].items():
        #     print(key)
        if average_policy:
            loaded_dict_1 = torch.load(path_1)
            loaded_dict_2 = torch.load(path_2)
            alpha = 0.5
            beta = 0.5
            gamma = 1 - alpha - beta

            for i in [0, 2, 4, 6]:
                loaded_dict['actor_state_dict']["actor.{}.weight".format(i)] = alpha * loaded_dict_1['actor_state_dict']["actor.{}.weight".format(i)] + \
                    beta * loaded_dict_2['actor_state_dict']["actor.{}.weight".format(i)] + \
                    gamma * loaded_dict['actor_state_dict']["actor.{}.weight".format(i)]
                loaded_dict['actor_state_dict']["actor.{}.bias".format(i)] = alpha * loaded_dict_1['actor_state_dict']["actor.{}.bias".format(i)] + \
                    beta * loaded_dict_2['actor_state_dict']["actor.{}.bias".format(i)] + \
                    gamma * loaded_dict['actor_state_dict']["actor.{}.bias".format(i)]

            for i in [0, 2, 4]:    
                loaded_dict['dm_encoder_state_dict']["encoder.{}.weight".format(i)] = alpha * loaded_dict_1['dm_encoder_state_dict']["encoder.{}.weight".format(i)] + \
                    beta * loaded_dict_2['dm_encoder_state_dict']["encoder.{}.weight".format(i)] + \
                    gamma * loaded_dict['dm_encoder_state_dict']["encoder.{}.weight".format(i)]
                loaded_dict['dm_encoder_state_dict']["encoder.{}.bias".format(i)] = alpha * loaded_dict_1['dm_encoder_state_dict']["encoder.{}.bias".format(i)] + \
                    beta * loaded_dict_2['dm_encoder_state_dict']["encoder.{}.bias".format(i)] + \
                    gamma * loaded_dict['dm_encoder_state_dict']["encoder.{}.bias".format(i)]                             
