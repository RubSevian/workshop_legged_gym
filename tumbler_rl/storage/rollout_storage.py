...
from tumbler_rl.utils import split_and_pad_trajectories
...
class RolloutStorage:
    ...
    def __init__(self, num_envs, num_transitions_per_env, obs_shape, privileged_obs_shape, actions_shape, Hist_info_shape, device='cpu'):
        self.device = device
        ...
        # For entrin loss
        self.extrin_loss = torch.zeros(num_transitions_per_env, num_envs, 6, device=self.device)

        # For privileged info
        self.privileged_info = torch.zeros(num_transitions_per_env, num_envs, privileged_obs_shape[0], device=self.device)
        ...
        for epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                yield obs_batch, critic_observations_batch, actions_batch, target_values_batch, advantages_batch, returns_batch, \
                      old_actions_log_prob_batch, old_mu_batch, old_sigma_batch, (None, None), None, privileged_info_batch,  proprio_hist_batch, \
                      extrin_loss
